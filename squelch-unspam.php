...
function lstunspam_rename_comment_field( $fields ) {
    ...
    /* NOTE: WordPress filters everything except the comments textarea for
     *       logged in users, so we need to use a separate action,
     *       comment_form_top, to add fields for those users.
     */
    if (is_user_logged_in()) {
        global $lstunspam_logged_in_comment_form_fields;
        $lstunspam_logged_in_comment_form_fields = $fields;

        add_filter( 'comment_form_top', 'lstunspam_add_hidden_fields_logged_in_users' );
    }
...
/**
 * Adds hidden fields on the form for logged in users.
 */
function lstunspam_add_hidden_fields_logged_in_users() {
    global $lstunspam_logged_in_comment_form_fields;

    $fields = $lstunspam_logged_in_comment_form_fields['fields'];

    foreach ( $fields as $name => $field ) {
        if (substr( $name, 0, 10 ) === 'lstunspam_') {
